...
export default async function EventRegistrationPage({ params }: Props) {
  try {
    ...
    return (
      <div className="container mx-auto p-12 text-destructive">
        <h1 className="text-4xl font-bold mb-4">Server Rendering Error</h1>
        <p className="text-xl mb-4">Message: {error.message}</p>
        <pre className="bg-destructive/10 p-4 rounded-xl overflow-auto whitespace-pre-wrap">
          {error.stack}
        </pre>
      </div>
    )
